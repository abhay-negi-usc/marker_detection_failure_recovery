# ~/isaacsim/python.sh ablations/synthetic_data_generation/background_sdg.py 
# ~/.local/share/ov/pkg/isaac-sim-4.5.0/python.sh ablations/synthetic_data_generation/background_sdg.py 


# DESCRIPTION: 
# synthetic data generation for ablation studies 
# 0) pose 
# 1) truncation  
# 2) underexposure 
# 3) backlighting 
...
config = {
    ...
    "simulation_duration_between_captures": 0, # FIXME 
    "resolution": (640, 480),
    ...
    "lights": "dome", # dome, distant_light 
}
#------------------------------------------------------------------------------------------------------------------------------------------------------#

# SET UP DIRECTORIES 
timestr = time.strftime("%Y%m%d-%H%M%S") 
out_folder = config.get("writer_kwargs", {}).get("output_folder", "./output_test")
OUT_DIR = os.path.join(out_folder, "exp_sdg_" + timestr) 

dir_textures = "./synthetic_data_generation/assets/tags/sdg_tag"
cwd = os.getcwd() 
sys.path.append(cwd)
# from ablations.synthetic_data_generation.utils import * 
...
def run_simulation_loop(duration):
    ...
    max_steps = 1 
    while elapsed_time <= duration and app_updates_counter < max_steps:
...
if config["lights"] == "dome": 
    ...
    dome_light_intensity_attr = dome_light.GetAttribute("inputs:intensity")
